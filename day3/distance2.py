...
def setDistance(distances, d, wire):
  if distances[wire] == -1 or distances[wire] < d:
    distances[wire] = d
  ...
  for (direction, distance) in segments:
    for d in range (1, distance+1):
      if direction == 'U':
        ...
        x += 1
      total_distance += 1
      setDistance(grid[(x, y)], total_distance, wire)

def closestIntersection(grid):
  return min(distances[0] + distances[1] for distances in grid.values() if distances[0] > 0 and distances[1] > 0)

def calcDistance(path1, path2):
  grid = defaultdict(lambda: [-1,-1])
